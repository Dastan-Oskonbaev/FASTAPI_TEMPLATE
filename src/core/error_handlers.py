...
async def http_exception_handler(request: Request, e: HTTPException):
    return error_response(code=e.detail or "HTTP_ERROR", message="An HTTP error occurred", status_code=e.status_code)


async def validation_exception_handler(request: Request, e: RequestValidationError):
    errors = e.errors()
    detail_messages = [{"field": err.get("loc")[-1], "issue": err.get("msg")} for err in errors]
